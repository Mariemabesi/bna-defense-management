...
@Component({
          ...
          <tbody>
            <tr *ngFor="let f of filteredFrais()">
              ...
              <td class="actions-group">
                ...
                <!-- BADGES FOR FINISHED STATES -->
                <span *ngIf="f.statut === 'VALIDE'" class="status-badge valid">Validé 🛡️</span>
                <span *ngIf="f.statut === 'REJETE'" class="status-badge error" [title]="f.observation">Refusé ❌</span>
                <span *ngIf="f.statut === 'ENVOYE_TRESORERIE'" class="status-badge treasury">Payé 🏦</span>
              </td>
            </tr>
          </tbody>
        </table>
  ...
  styles: [`
    ...
    .status-badge.error { color: #dc2626; font-weight: 800; cursor: help; }
    .status-badge.treasury { color: #2563eb; font-weight: 800; }
    ...
    .btn-action.validate { background: #eff6ff; color: #1d4ed8; border: 1px solid #bfdbfe; }
    .btn-action.validate:hover { background: #1d4ed8; color: white; }
